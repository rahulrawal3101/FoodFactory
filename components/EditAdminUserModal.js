'use client'
import { Box, Button, Checkbox, Grid, Modal, Paper, Typography } from '@mui/material';
...
const EditAdminUser = ({ openEdit, setOpenEdit,  fetchUserApi }) => {
    ...
    // console.log('open edit modal',openEdit.details)
    const handleCollect = (e) => {
        ...
        if( value != openEdit.details[name] && e.target.checked != openEdit.details[name] ){
    ...
    useEffect(() => {
        setInputData({ ...inputData, fullName: openEdit.details.fullName, mobile: openEdit.details.mobile, email: openEdit.details.email,isActive:openEdit.details.isActive })
    }, [openEdit]);
...
// console.log(inputData)
    ...
    const submitHandler = async () => {
        ...
        if (inputData.fullName != '' && inputData.mobile != '' && inputData.email != '') {
            try {
                const res = await axios.patch(`/api/edituserdetails/${openEdit.details._id}`, { fullName: inputData.fullName, mobile: inputData.mobile, email: inputData.email,isActive:inputData.isActive  });
                console.log(res);
                if (res.data.message == 'UserDetails Edited Successfully') {
                    alert(res.data.message);
                    fetchUserApi();
                    setOpenEdit({...openEdit,open:false})
                    setLoaders({...loaders,loading:false,isDisabled:true})
                }
                if(res.data.message =='Failed To Edit UserDetails'){
    ...
    return (
        <>
            <Modal open={openEdit.open} disableAutoFocus onClose={handleClose} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
                    ...
                    <Box sx={{ marginTop: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <Box sx={{ mt: 3 }}>
                            <Grid container spacing={2}>
                                ...
                                <Typography sx={{fontSize:'15px'}}>IsActive : <Checkbox  name="isActive" checked={inputData.isActive  || false} onChange={handleCollect}/> </Typography>
                                </Grid>
